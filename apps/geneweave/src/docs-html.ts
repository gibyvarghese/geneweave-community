...
${callout('warn', '🧭', 'What runs on Postgres today.', 'On Postgres, geneWeave creates the <strong>entire</strong> database schema (every table), and <strong>almost every area</strong> is implemented at <strong>full parity with SQLite</strong>: <code>users</code>, <code>chats</code>, <code>messages</code>, <code>skills</code>, <code>cost</code>, <code>capabilities</code>, <code>voice</code>, <code>workflows</code>, <code>scopes</code>, <code>agents</code>, <code>prompts</code>, <code>tools</code>, <code>routing</code>, <code>memory</code>, <code>encryption</code>, <code>agenda/notes</code>, <code>kaggle</code>, and <code>live-agents</code> — with only the admin console and current-user (<code>me</code>) areas still in progress. Areas not yet ported raise a clear, self-explaining error the moment they’re called (never a silent wrong answer), so it’s always obvious what’s ready. For complete coverage today, stay on SQLite (the default); the remaining areas land incrementally.')}
