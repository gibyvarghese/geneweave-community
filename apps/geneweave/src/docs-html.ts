...
${callout('warn', '🧭', 'What runs on Postgres today.', 'On Postgres, geneWeave creates the <strong>entire</strong> database schema (every table) and implements <strong>every area</strong> at <strong>full parity with SQLite</strong> — users, chats, messages, skills, cost, capabilities, voice, workflows, scopes, agents, prompts, tools, routing, memory, encryption, agenda/notes, kaggle, live-agents, the admin console, and current-user (<code>me</code>) context. The one piece still SQLite-only is the first-run <em>default-data seeding</em> (the demo skills, agents, routing policies, etc.); everything else — all reads and writes — runs on Postgres. Areas not yet ported raise a clear, self-explaining error the moment they’re called (never a silent wrong answer), so it’s always obvious what’s ready. For complete coverage today, stay on SQLite (the default); the remaining areas land incrementally.')}
...
${callout('info', '🛡️', 'Optional second line of defence — Postgres row-level security.', 'The app already scopes every query to the right tenant — that is what keeps tenants isolated. On Postgres you can turn on an <strong>independent backstop</strong> at the database itself: set <code>GENEWEAVE_PG_RLS=1</code> and every tenant-scoped table gets a row-security policy. It is inert for normal operation (admin and cross-tenant reads still see everything), but code that runs inside a tenant scope literally <em>cannot</em> read or write another tenant’s rows — even a query that explicitly asked for them comes back empty. So if an application query ever forgot its tenant filter, Postgres would still refuse to leak the data. It is off by default and Postgres-only (SQLite installs are unaffected); the app-level scoping remains the correctness mechanism and this is defence in depth on top.')}
`)}
